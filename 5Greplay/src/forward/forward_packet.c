...
/**
 * @brief Modify attribute in a DICOM packet
 *
 * @param data
 * @param data_size
 * @param ipacket
 ...
 */
uint32_t update_dicom_data( char *data, uint32_t data_size, const ipacket_t *ipacket, uint32_t proto_id, uint32_t att_id, uint32_t new_val){
	uint32_t ret = 0;
	fprintf(stderr, "Going to update the value of attribute %d, new value : %lu (%lx)\n",att_id, new_val, new_val);
	debug("DICOM: update the value of attribute %d, new value : %lu (%lx)",att_id, new_val, new_val);
	if( proto_id != 701 )
		return ret;
