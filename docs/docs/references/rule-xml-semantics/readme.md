...
A NetworkFuzzer rule is a XML file that can contain as many events as required to filter the desired packets to apply the actions defined by the properties (i.e forwarding or dropping it, with or without modification)
...
![NetworkFuzzer property structure](rule-struct.png "NetworkFuzzer property structure")
...
Thus, if NetworkFuzzer is configured with the default action in the config file is forward, then it forwards the traffic that does not come from 192.168.0.15.
