...
We can also get this list by running `./networkfuzzer list`.
...
Protocol `nas_5g` having 6 *main* attributes and 9 *metadata* attributes which are used internally by DPI engine of NetworkFuzzer.
...
on wich NetworkFuzzer can parse the  NGAP, NAS-5G and DIAMETER protocols.
