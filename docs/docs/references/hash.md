...
- A `unint_64 hash` number is used to present a set of proto.atts by turning on the corresponding bit of each proto.att.
  ...
  This is calculated only once, when initializing NetworkFuzzer, by the function `_calculate_hash_number`.
...
- A rule has a hash number representing its requirement of proto.atts. 
  This number is calculated only once, when initializing NetworkFuzzer, inside the function `mmt_single_sec_register`.
