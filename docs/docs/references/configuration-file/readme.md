...
NetworkFuzzer requires a configuration file for setting different options:

- Online or offline mode
- Default action to be taken with the non-filtered packets

By default, NetworkFuzzer will try to load the configuration from `./networkfuzzer.conf` file in the current folder. 
A configuration file can be given to NetworkFuzzer by using `-c` parameter, for example:
`./networkfuzzer replay -c /home/tata/probe.conf`
...
`./networkfuzzer replay -c /home/tata/smc-replay-attack.conf -Xinput.source=/tmp/a.pcap`

To list the attributes that can be overriden, run: `./networkfuzzer replay -x`
...
This block configures the input of NetworkFuzzer.

- `mode` can be either `ONLINE` or `OFFLINE` to indicate that NetworkFuzzer will analyze respectively either traffic in realtime from a NIC or the traffic being stocked in a pcap file.
...
*Note*: Disable reports output can gain some execution performance of NetworkFuzzer.

## 3. `engine` block

The engine configuration block allows configure functionalities related with the NetworkFuzzer rules.

- `thread-nb`: The option indicates the number of threads that NetworkFuzzer will use for processing packets. 
...
The forward configuration block configures parameters related with the forwarding capability of NetworkFuzzer.
