...
ZIP_NAME  ?= networkfuzzer-$(VERSION)_$(SYS_NAME)_$(SYS_VERSION).tar.gz
DIST_NAME ?= networkfuzzer-$(VERSION)

dist: sample-rules
	@[ "${STATIC_LINK}" ] || ( echo ">> STATIC_LINK is not set. Do 'make STATIC_LINK=1 dist"; exit 1 )
	$(MKDIR) $(DIST_NAME)
	$(CP) /opt/mmt/plugins ./rules networkfuzzer.conf $(OUTPUT) $(DIST_NAME)/
