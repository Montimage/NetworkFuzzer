 ...
 *
 * This function will cause NetworkFuzzer to exclude proto_att from its mask, i.e.,
...
extern void mmt_set_attribute_string_value(uint32_t, uint32_t, u_char *); //this function must be implemented inside mmt-probe
extern int replace_dicom_attribute(uint32_t proto_id, uint32_t att_id, const void *new_val, int is_string);
...
static inline uint32_t get_parameter_nb_copies(){
	const char *str = getenv("NETWORKFUZZER_NB_COPIES");
	if( str == NULL )
		return 0;
	else
...
static inline uint32_t get_http2_env_nb_copies(){
	const char *str = getenv("NETWORKFUZZER_HTTP2_NB_COPIES");
	if( str == NULL )
		return 5000;
	else
		return strtoul( str, NULL, 0 );
