...
uint16_t _mmt_sec_hash_proto_attribute_without_lock( uint32_t proto_id, uint32_t att_id ){
	...
	ABORT( "Attribute %d.%d has not been registered in NetworkFuzzer", proto_id, att_id );
	return 0;
}
...
mmt_sec_handler_t* mmt_sec_register( size_t threads_count, const uint32_t *cores_id, const char *rules_mask,
	...
	if( verbose ){
		if( threads_count == 0 )
			log_write( LOG_INFO, "NetworkFuzzer %s is verifying %zu rules having %zu proto.atts using the main thread",
				mmt_sec_get_version_info(),
				rules_count, proto_atts_count );
		else
			log_write( LOG_INFO, "NetworkFuzzer %s is verifying %zu rules having %zu proto.atts using %zu threads",
							mmt_sec_get_version_info(),
