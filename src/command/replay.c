...
void register_signals(){
	...
	// => we need to capture this signal to avoid quiting NetworkFuzzer
...
#define MAX_ENV_STRING_LEN 1024
	...
	snprintf( environment, MAX_ENV_STRING_LEN, "NETWORKFUZZER_NB_COPIES=%u", config->forward->nb_copies );
	snprintf( environment, MAX_ENV_STRING_LEN, "NETWORKFUZZER_HTTP2_NB_COPIES=%u", config->forward->nb_copies );
	putenv( environment );
